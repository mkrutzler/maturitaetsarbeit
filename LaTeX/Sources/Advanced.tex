...
In the end we still end up with many variables like the allotment time per queue, the amount of queues, the quanta for the Round Robin and others.
...
To summarize the Multi-Level Feedback Queue I would like to quote the rules of it from the book OStep \cite{ostep}:
\begin{quote}
\emph{
\begin{enumerate}
        ...
        \item When a job enters the system, it si placed at the highest priority
...
One important difference between a currency in real life and tickets is that tickets are not consumed when used.
